...
import { ArrowLeft, Plus, Trash2, Loader2, AlertTriangle, Search, IndianRupee } from "lucide-react";
import { formatCurrency } from "@/lib/utils";
import type { Customer, AnomalyResult } from "@/types";
  ...
  async function checkAnomalies() {
    try {
      const res = await fetch("/api/anomaly", {
        ...
        body: JSON.stringify({ invoice: { customer_id: customerId, items, total_amount: totalAmount } }),
      });
      const data = await res.json();
  ...
  async function handleSubmit(status: "draft" | "sent") {
    ...
    try {
      const res = await fetch("/api/invoices", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ customer_id: customerId, invoice_date: invoiceDate, due_date: dueDate, status, items, notes }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || data.details?.join(", ") || "Failed to create invoice");
    ...
    <div className="space-y-6 max-w-5xl">
        ...
        <div className="space-y-2">
          {anomalies.map((a, i) => (
            <div key={i} className={`flex items-start gap-2 p-3 rounded-md text-sm ${a.severity === "high" ? "bg-destructive/10 text-destructive" : a.severity === "medium" ? "bg-amber-500/10 text-amber-600" : "bg-muted text-muted-foreground"}`}>
              ...
              <div><strong>{a.field}:</strong> {a.issue}. <em>{a.suggestion}</em></div>
            </div>
          ))}
        </div>
      )}
        ...
        <CardContent className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="space-y-2">
            ...
            <Select value={customerId} onValueChange={setCustomerId}>
              ...
              <SelectContent>
                {customers.map((c) => (
                  <SelectItem key={c.id} value={c.id}>{c.name}{(c as any).company_name ? ` - ${(c as any).company_name}` : ""}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
                  ...
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="relative">
                      <Input
                        ...
                        onFocus={() => { setActiveItemIdx(idx); setHsnSearch(item.hsn_code); searchHSN(item.hsn_code); }}
                        placeholder="HSN"
                        className="border-0 px-0"
                      />
                        ...
                        <div className="absolute top-full left-0 z-20 w-80 bg-popover border rounded-md shadow-lg p-1 mt-1 max-h-40 overflow-y-auto">
                          {hsnResults.map((h) => (
                            <button key={h.hsnCode} className="w-full text-left px-3 py-2 text-sm hover:bg-accent rounded-sm flex items-center justify-between" onClick={() => selectHSN(idx, h)}>
                              ...
                              <Badge variant="secondary" className="text-xs">{h.hsnCode} | {h.gstRate}%</Badge>
                            </button>
                          ))}
                        </div>
                      )}
                    ...
                    <TableCell>
                      <Input type="number" min={1} value={item.quantity} onChange={(e) => updateItem(idx, "quantity", +e.target.value)} className="border-0 px-0 text-right" />
                    </TableCell>
                    <TableCell>
                      <Input type="number" min={0} step="0.01" value={item.unit_price || ""} onChange={(e) => updateItem(idx, "unit_price", +e.target.value)} className="border-0 px-0 text-right" />
                    </TableCell>
                    <TableCell>
                      <Select value={String(item.gst_rate)} onValueChange={(v) => updateItem(idx, "gst_rate", +v)}>
                        <SelectTrigger className="border-0 h-8 text-right"><SelectValue /></SelectTrigger>
                        <SelectContent>
      ...
      <div className="flex flex-col sm:flex-row justify-end gap-6">
        ...
        <div className="flex flex-col gap-2">
          <Button onClick={checkAnomalies} variant="outline" size="sm"><AlertTriangle className="h-4 w-4 mr-1" />Check Anomalies</Button>
